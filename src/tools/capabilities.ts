...
export const DEVELOPER_CAPABILITIES: readonly DeveloperCapability[] = [
  {
    ...
    requestVia:
      "Set provisionS3Bucket: true in the service's chart/values.yaml. The shared chart renders chart/templates/s3-claim.yaml, which creates an XS3Bucket claim; Crossplane reconciles it into a real bucket. The Backstage templates expose the same flag as a checkbox at scaffold time.",
    implementation:
      'platform-demo-gitops/apps/crossplane/compositions/ (XS3Bucket XRD + AWS Composition), rendered by platform-demo-hello-world-template/common/chart/templates/s3-claim.yaml',
    doNot:
  ...
  },
  {
    ...
    requestVia:
      "resources.requests.cpu, resources.requests.memory and resources.limits.memory in the service's chart/values.yaml, with replicaCount alongside them. Kyverno's require-resource-limits refuses a pod that omits any of the three.",
    implementation:
      'platform-demo-hello-world-template/common/chart/values.yaml and the rollout template that consumes it',
    doNot:
  ...
  },
  {
    ...
    requestVia:
      'autoscaling.enabled, minReplicas, maxReplicas and targetCPUUtilizationPercentage in chart/values.yaml, rendered as a HorizontalPodAutoscaler.',
    implementation: 'platform-demo-hello-world-template/common/chart/templates/hpa.yaml',
    doNot:
  ...
  },
  {
    ...
    requestVia:
      'rollout.canary.steps in chart/values.yaml. The default is 20 → 50 → 100 with an istio-success-rate AnalysisTemplate that aborts the rollout on an error-rate regression.',
    implementation:
      'platform-demo-gitops/apps/argo-rollouts/ and platform-demo-hello-world-template/common/chart/templates/rollout.yaml',
    doNot:
  ...
  },
  {
    ...
    requestVia:
      'The OpenTelemetry contract only: otel.endpoint, otel.protocol and otel.serviceNamespace in chart/values.yaml, pointing at the collector gateway. Logs are written to stdout and collected by the agent DaemonSet — the application does not export them.',
  ...
  },
  {
    ...
    requestVia:
      "A PrometheusRule. Prometheus discovers rules in every namespace (ruleSelectorNilUsesHelmValues: false), so a new rule needs no configuration change to be picked up. A rule about one service belongs in that service's chart/templates/; a rule every service should have belongs in common/chart/templates/ in the templates repository.",
    implementation:
  ...
  },
  {
    ...
    requestVia:
      'istio.gatewayHost in chart/values.yaml renders the VirtualService and DestinationRule. Mutual TLS is already on for everything: PeerAuthentication is STRICT mesh-wide and needs nothing from the service.',
    implementation:
  ...
  },
  {
    ...
    implementation:
      'platform-demo-hello-world-template/templates/*/skeleton/.github/workflows/ci.yml, enforced at admission by require-signed-images and require-sbom-attestation',
    doNot:
